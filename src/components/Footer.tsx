  ...
  return (
    <footer className="bg-white px-4 md:px-8 pb-0 pt-0 border-t border-gray-100">
      <div className="container mx-auto max-w-7xl">
        ...
        {/* --- SINGLE RED BAR (Icons + Copyright) --- */}
        <div className="w-full bg-[#CE1126] rounded-t-2xl md:rounded-2xl p-3 md:p-4 flex flex-col md:flex-row items-center justify-between relative overflow-hidden mt-6 md:mb-6">
             {/* Decorative Circles */}
...
const SocialIcon = ({ href, icon: Icon, label }: { href: string; icon: any; label: string }) => (
  <a
    ...
    className="w-8 h-8 md:w-9 md:h-9 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white hover:text-[#CE1126] transition-all duration-300 group"
    aria-label={label}
  >
    <Icon className="w-4 h-4 md:w-5 md:h-5 text-white group-hover:text-[#CE1126] transition-colors" />
  </a>
);
