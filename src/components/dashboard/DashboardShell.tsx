...
export function DashboardShell({ navItems, activeTab, onTabChange, accentColor, roleLabel, children }: DashboardShellProps) {
  ...
  const sidebarContent = (isMobile: boolean) => (
    <>
      ...
      {/* Nav search — expanded sidebar only */}
      {(isMobile || !collapsed) && (
        <div className="px-3 pt-2.5 pb-1 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 placeholder:text-slate-400 focus:bg-white focus:border-slate-300 focus:outline-none transition-all"
            />
          </div>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      {/* Desktop top bar */}
      <div className={`hidden lg:flex fixed top-0 right-0 ${mainMargin} left-0 h-12 bg-white/80 backdrop-blur-lg border-b border-slate-200/60 z-20 items-center px-6 gap-4 transition-all duration-300`} style={{ marginLeft: collapsed ? 68 : 240 }}>
