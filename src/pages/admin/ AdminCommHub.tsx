...
import {
  ...
  XCircle, Plus, ChevronRight, ChevronDown, ChevronUp, AlertCircle,
  Search, Users, Megaphone, Pin, X, Loader2, Phone, Filter,
  FileText, Mail, MapPin, ChevronLeft, Inbox, MailOpen, Circle
} from 'lucide-react';
...
/* ------------------------------------------------------------------ */
/*  IMPORTANT: SMS Integration Placeholder                             */
/*  Replace the sendViaSMS function body with your actual SMS API call */
/*  when you're ready to connect your provider.                        */
/* ------------------------------------------------------------------ */
async function sendViaSMS(
  ...
  // ── PLACEHOLDER ──────────────────────────────────────────────
  // Replace this block with your real SMS API integration.
  //
  // Example shape for most Ghana SMS APIs:
  //
  ...
  //
  // OR call a Supabase Edge Function:
  //
...
export function AdminCommHub() {
  ...
  const loadSmsHistory = useCallback(async () => {
    // SMS records stored in messages table with message_type = 'sms'
  ...
  const loadTemplates = useCallback(async () => {
    ...
    if (data) {
      setTemplates(data.map(d => ({
        ...
        name: d.title,
        content: d.body,
        category: d.target_role === '__template__' ? 'general' : d.target_role,
      })));
    }
  }, []);
  ...
  const filteredSmsHistory = useMemo(() => {
    ...
    return smsHistory.filter(m =>
      m.recipient_phone.includes(historySearch) ||
  ...
  const handleSend = async () => {
    ...
    if (channel === 'sms' && isOverCharLimit) return;
    setSending(true);
    setSendResult(null);
    ...
    try {
      if (channel === 'sms') {
        ...
        if (!error) {
          setSendResult({ type: 'success', text: `Message sent to ${selectedRecipients.length} recipient${selectedRecipients.length !== 1 ? 's' : ''}` });
        } else {
          setSendResult({ type: 'error', text: 'Failed to send messages' });
        }
      }
      setMessageContent('');
      setSubject('');
        ...
        {([
          ...
          return (
            <button
              ...
              className={`flex-1 py-2 text-[13px] font-bold rounded-lg transition-all duration-200 ${
                isActive ? 'text-white shadow-md' : 'text-slate-500 hover:text-slate-700'
              }`}
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-1 flex">
            {(['sms', 'in-app'] as ComposeChannel[]).map(ch => (
              <button
                key={ch}
                onClick={() => { setChannel(ch); clearSelection(); }}
                className={`flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all ${
                  channel === ch ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
          ...
          <AnimatePresence>
            {sendResult && (
              ...
              >
                ...
                <p className={`text-sm font-bold flex-1 ${sendResult.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>
                  {sendResult.text}
                </p>
                <button onClick={() => setSendResult(null)} className="p-1 hover:bg-white/50 rounded-lg">
            ...
            <div className="p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="text-[11px] font-black uppercase tracking-wider flex items-center gap-1 hover:opacity-80 transition-opacity"
                  style={{ color: ACCENT }}
                >
                  <Users className="w-3.5 h-3.5" />
                  {showRecipientPicker ? 'Close' : 'Choose'}
                </button>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedRecipients.slice(0, 8).map(r => (
                    <span key={r.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-[11px] font-bold rounded-lg" style={{ color: ACCENT }}>
                      {r.full_name}
                      <button onClick={() => toggleRecipient(r.id)} className="hover:bg-red-100 rounded p-0.5">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  ))}
                  ...
                  <div className="border-t border-slate-100 p-4 space-y-3 bg-slate-50/50">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className="w-full pl-9 pr-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-slate-300"
                        />
                      </div>
                      <select
                        value={zoneFilter}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <button onClick={selectAll} className="text-[11px] font-bold px-3 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-slate-50">
                          Select All ({filteredRecipients.length})
                        </button>
                        {selectedIds.size > 0 && (
                          <button onClick={clearSelection} className="text-[11px] font-bold px-3 py-1 rounded-lg text-red-600 bg-red-50 hover:bg-red-100">
                            Clear
                          </button>
                        )}
                      </div>
                    ...
                    <div className="max-h-[240px] overflow-y-auto bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
                      {filteredRecipients.length === 0 ? (
                        <div className="p-6 text-center text-slate-400 text-sm font-medium">
                          {channel === 'sms' ? 'No recipients with phone numbers found' : 'No recipients found'}
                        </div>
                      ) : (
                        filteredRecipients.map(r => {
                          ...
                          return (
                            <label
                              key={r.id}
                              className={`flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors ${checked ? 'bg-red-50/40' : 'hover:bg-slate-50'}`}
                            >
                              <input
                                type="checkbox"
                                checked={checked}
                                onChange={() => toggleRecipient(r.id)}
                                className="w-4 h-4 rounded border-slate-300 accent-[#CE1126]"
                              />
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            ...
            {channel === 'in-app' && (
              <div className="p-5 border-b border-slate-100">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm font-medium text-slate-900 outline-none border border-slate-100 focus:ring-2 focus:ring-red-100 focus:border-red-200 transition-all"
                />
              </div>
            )}
            ...
            <div className={`p-5 transition-colors ${isOverCharLimit ? 'bg-red-50/30' : ''}`}>
              <div className="flex items-center justify-between mb-2">
                <label className={`text-[11px] font-black uppercase tracking-wider ${isOverCharLimit ? 'text-red-600' : 'text-slate-500'}`}>
                  Message
                </label>
                {channel === 'sms' && (
                  <span className={`text-[10px] font-black uppercase tracking-wider ${isOverCharLimit ? 'text-red-600' : 'text-slate-400'}`}>
              ...
              <textarea
                ...
                placeholder={channel === 'sms' ? 'Type your SMS message (max 160 chars)...' : 'Type your message...'}
                rows={channel === 'sms' ? 4 : 6}
                className={`w-full px-4 py-3 bg-slate-50 rounded-xl text-sm font-medium outline-none border border-slate-100 resize-none focus:ring-2 transition-all ${
                  isOverCharLimit ? 'text-red-900 focus:ring-red-200 focus:border-red-200' : 'text-slate-900 focus:ring-red-100 focus:border-red-200'
                }`}
              />
              {isOverCharLimit && (
                <div className="flex items-center gap-2 mt-3 p-2.5 bg-red-100 rounded-xl text-red-700">
                  <AlertCircle className="w-4 h-4 shrink-0" />
                  <span className="text-[11px] font-bold">Message exceeds {SMS_CHAR_LIMIT} character SMS limit</span>
                </div>
              )}
            </div>
          ...
          <button
            onClick={handleSend}
            disabled={sending || selectedIds.size === 0 || !messageContent.trim() || isOverCharLimit}
            className="w-full py-4 rounded-2xl text-[15px] font-bold text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] transition-all flex items-center justify-center gap-2"
          ...
          {selectedInApp ? (
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
              <div className="flex items-center gap-2 p-4 border-b border-slate-100">
                <button onClick={() => setSelectedInApp(null)} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors">
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-1 flex">
                <button
                  ...
                  className={`flex-1 py-2 text-[12px] font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                    historyTab === 'sms' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  <Phone className="w-3.5 h-3.5" /> SMS Log
                </button>
                <button
                  onClick={() => setHistoryTab('in-app')}
                  className={`flex-1 py-2 text-[12px] font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${
                    historyTab === 'in-app' ? 'bg-slate-900 text-white' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  <Mail className="w-3.5 h-3.5" /> In-App
                </button>
              </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                  {historyLoading ? (
                    ...
                    <div className="p-12 text-center">
                      ...
                      <p className="font-bold text-slate-400">No messages</p>
                    </div>
                  ) : (
                    <div className="divide-y divide-slate-100">
                      {inAppMessages.map(msg => (
                        <button
                          key={msg.id}
                          onClick={() => openInAppDetail(msg)}
                          className={`w-full text-left px-4 py-3 hover:bg-slate-50 transition-colors flex items-start gap-3 ${
                            !msg.is_read && msg.recipient_id === user?.id ? 'bg-red-50/30' : ''
                          }`}
                        >
                          <div className="mt-1.5">
                            {msg.is_read || msg.sender_id === user?.id
                              ? <MailOpen className="w-4 h-4 text-slate-300" />
                              : <Circle className="w-3 h-3 fill-current" style={{ color: ACCENT }} />
                            }
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <p className={`text-sm truncate ${!msg.is_read && msg.recipient_id === user?.id ? 'text-slate-900 font-bold' : 'text-slate-600'}`}>
                                {msg.sender_id === user?.id ? `To: ${msg.recipient_id?.slice(0, 8)}...` : msg.sender_name}
                              </p>
      ...
      {activeView === 'templates' && (
        <div className="space-y-4">
          <button
            ...
            className="w-full py-4 rounded-2xl bg-white border-2 border-dashed border-slate-200 font-bold text-[14px] flex items-center justify-center gap-2 hover:border-red-200 hover:bg-red-50/30 transition-all group"
            style={{ color: ACCENT }}
          >
                ...
                <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Community Meeting Reminder"
                      className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm font-medium text-slate-900 outline-none border border-slate-100 focus:ring-2 focus:ring-red-100"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 bg-slate-50 rounded-xl text-sm font-medium text-slate-900 outline-none resize-none border border-slate-100 focus:ring-2 focus:ring-red-100"
                    />
                  </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            {templates.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-xs text-slate-300 mt-1">Create reusable message templates</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {templates.map(t => (
                  <div key={t.id} className="p-5 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                    ...
                    <div className="flex items-center gap-2 ml-3 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200"
                      >
                        Use
                      </button>
                      <button
                        onClick={() => deleteTemplate(t.id)}
                        className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100"
                      >
                        Delete
                      </button>
                    </div>
      ...
      {activeView === 'announcements' && (
        <div className="space-y-4">
          <button
            ...
            className="w-full py-4 rounded-2xl bg-white border-2 border-dashed border-slate-200 font-bold text-[14px] flex items-center justify-center gap-2 hover:border-amber-200 hover:bg-amber-50/30 transition-all"
            style={{ color: '#D97706' }}
          >
                ...
                <div className="bg-white rounded-2xl border border-amber-200 p-5 space-y-4 bg-amber-50/30">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white rounded-xl text-sm font-medium text-slate-900 outline-none border border-amber-200 focus:ring-2 focus:ring-amber-100"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 bg-white rounded-xl text-sm font-medium text-slate-900 outline-none resize-none border border-amber-200 focus:ring-2 focus:ring-amber-100"
                    />
                  </div>
                  <div className="flex items-center gap-3">
                    <select
                      ...
                      className="px-3 py-2.5 bg-white border border-amber-200 rounded-lg text-sm font-medium focus:outline-none"
                    >
                    ...
                    <button
                      ...
                      className="ml-auto px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm rounded-xl disabled:opacity-50 transition-colors flex items-center gap-1.5"
                    >
          ...
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
            {announcementsLoading ? (
              ...
              <div className="p-12 text-center">
                ...
                <p className="font-bold text-slate-400">No announcements</p>
              </div>
            ) : (
              <div className="divide-y divide-slate-100">
                {announcements.map(a => (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              onClick={() => togglePin(a)}
                              className={`text-[10px] font-bold px-2 py-0.5 rounded ${a.is_pinned ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400 hover:bg-amber-50 hover:text-amber-500'}`}
                            >
