...
import {
  TrendingUp, Users, CreditCard, Smartphone,
  Download, ArrowUpRight, ArrowDownRight,
  ArrowLeft, ShieldCheck, Copy, CheckCircle2,
  Target, Sparkles, Clock, Hash, Award
} from 'lucide-react';
...
export function AdminFinancials({ contributions, projects }: AdminFinancialsProps) {
  ...
  const filteredByPeriod = useMemo(() => {
    ...
    return verifiedContributions.filter(c => (now - new Date(c.created_at).getTime()) < days * 86400000);
  }, [verifiedContributions, period]);

  const prevPeriod = useMemo(() => {
    const now = Date.now();
    const days = period === '7d' ? 7 : period === '30d' ? 30 : period === '90d' ? 90 : Infinity;
    if (days === Infinity) return [];
    return verifiedContributions.filter(c => {
  ...
  const exportCSV = () => {
    ...
    a.download = `donations-verified-${period}-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  };
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
          <div className="p-4 border-b border-slate-100 flex items-center justify-between">
            ...
            <span className="flex items-center gap-1 text-[10px] font-bold text-green-600"><ShieldCheck className="w-3 h-3" />Verified</span>
          </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full text-green-600 bg-green-50">
