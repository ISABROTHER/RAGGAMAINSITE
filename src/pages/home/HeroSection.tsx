...
export function HeroSection() {
  ...
  return (
    <section className="relative w-full h-[75vh] md:h-[92vh] overflow-hidden bg-slate-900">
      ...
      {/* Stats - Clean and without the dark bar */}
      <div className="absolute bottom-6 left-0 right-0 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-3 md:flex md:gap-16">
          {STATS.map((stat) => (
            <div key={stat.label} className="text-center md:text-left">
              <AnimatedCounter target={stat.value} suffix={stat.suffix} />
              <p className="text-[9px] md:text-[10px] text-white/60 font-bold uppercase tracking-wider mt-0.5">
                {stat.label}
              </p>
            </div>
          ))}
      ...
      <div className="absolute bottom-28 right-4 md:right-8 flex flex-col gap-1.5 z-20">
