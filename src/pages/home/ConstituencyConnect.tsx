    ...
    <div className="py-8 px-6 text-center relative overflow-hidden rounded-xl bg-slate-50 border border-slate-100">
      ...
      <div className="relative z-20 flex flex-col items-center gap-4">
        ...
        <div className="space-y-1">
            <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Searching Database...</p>
            <p className="text-[9px] text-green-600 font-mono tracking-tight">Accessing Talent Hub</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 text-center">
        <div className="mb-10">
          ...
          <p className="mt-4 text-green-50 text-sm md:text-base leading-relaxed max-w-lg mx-auto font-medium opacity-90">
            Join the database so we can build a constituency together and support you when opportunities arise.
          </p>
        </div>
                  ...
                  <motion.div key="search" {...anim} className="p-5">
                    <div className="flex items-center gap-3 mb-5">
                      <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center shrink-0">
                        {showInfo ? <Info className="w-5 h-5 text-white" /> : <Fingerprint className="w-5 h-5 text-white" />}
                      </div>
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                            <p className="text-sm font-black text-slate-900 uppercase tracking-tight">
                                {showInfo ? "Constituent Info" : "Why Register?"}
                            </p>
                            <div className={`w-1.5 h-1.5 rounded-full ${showInfo ? 'bg-slate-400' : 'bg-green-500'}`} />
                        </div>
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                            {showInfo ? "Tap to close" : "Click to find out"}
                        </p>
                      </button>
                    </div>
                      ...
                      {showInfo ? (
                        <motion.div key="info" {...anim} className="overflow-hidden mb-2">
                          <div className="bg-slate-50 rounded-xl p-4 space-y-4 text-slate-800 text-[11px] leading-relaxed border border-slate-100 text-left">
                            <div>
                              <h4 className="font-bold text-green-700 uppercase text-[10px] mb-1">Purpose & Benefits</h4>
                              <p>We confirm you are a constituent to respond to issues quickly. You get informed when support or opportunities are available.</p>
                            </div>
                            <div>
                              <h4 className="font-bold text-green-700 uppercase text-[10px] mb-1">Privacy</h4>
                              <p>Information is used only for constituency work. We do not sell, publish, or share it for marketing.</p>
                            </div>
                            <button 
                                onClick={() => setShowInfo(false)}
                                className="w-full py-2.5 bg-slate-900 text-white font-black text-[10px] uppercase rounded-lg"
                            >
                                Back to Access
                            </button>
                          </div>
                          ...
                          {!activeTab && (
                              <div className="grid grid-cols-2 gap-3">
                                ...
                                >
                                    ...
                                    <span className="text-xs font-black uppercase tracking-wider">Join</span>
                                </button>
                              </div>
                          )}
                              ...
                              <div className="relative">
                                  ...
                                  <form onSubmit={handleSearch}>
                                    ...
                                    <button
                                      type="submit"
                                      disabled={!query.trim()}
                                      className="mt-3 w-full bg-slate-900 text-white font-black py-3 rounded-xl text-[10px] uppercase tracking-widest"
                                    >
                                      Verify Status
                                    </button>
                                  </form>
                            ...
                            {notFound && (
                              <motion.div key="nf" {...anim} className="mt-4 bg-amber-50 border border-amber-200/60 p-4 rounded-lg text-left">
                                <div className="flex items-center gap-3">
                                  ...
                                  <p className="font-bold text-amber-900 text-xs uppercase">No record for "{searchQuery}"</p>
                                </div>
