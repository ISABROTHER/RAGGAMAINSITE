...
import {
  LayoutDashboard, MessageSquare, Megaphone, AlertTriangle,
  CalendarDays, Users, TrendingUp, MapPin
} from 'lucide-react';
...
const NAV = [
  ...
  { id: 'issues', label: 'Constituency Issues', icon: AlertTriangle },
...
export function AssemblymanDashboard() {
  ...
  const quickStats = [
    ...
    { label: 'My Zone', value: profile?.zone || 'N/A', icon: MapPin, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'Engagement', value: '92%', icon: TrendingUp, color: 'text-teal-600', bg: 'bg-teal-50' },
  ];
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          <div>
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">
              Welcome, {profile?.full_name?.split(' ')[0] || 'Assembly Member'}
            </h2>
            <p className="text-slate-500 mt-1.5 font-medium">
              {profile?.zone ? `Serving ${profile.zone}` : 'Constituency Dashboard'}
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-5">
            <h3 className="font-bold text-slate-900 mb-4">Recent Constituency Issues</h3>
            {issues.length === 0 ? (
              <p className="text-sm text-slate-400 italic">No issues reported</p>
            ) : (
              <div className="space-y-3">
                {issues.slice(0, 4).map(issue => (
                  <div key={issue.id} className="flex items-start justify-between gap-3 p-3 bg-slate-50 rounded-xl">
                    <div className="min-w-0 flex-1">
                      ...
                      {issue.location && (
                        <div className="flex items-center gap-1 mt-1">
                          <MapPin className="w-3 h-3 text-slate-400" />
                          <p className="text-[10px] text-slate-400">{issue.location}</p>
                        </div>
                      )}
                    </div>
